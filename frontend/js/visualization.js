...
function loadVizPlayerList() {
    fetch(API_BASE+'/statistics/players/from-matches').then(function(r){return r.json();}).then(function(players){
        ...
        sel.innerHTML = '<option value="">-- 选择运动�?--</option>' +
...
function loadVizPlayerDetailed() {
    ...
    if (!name) { cont.innerHTML = '<div class="empty-state" style="padding:20px;">请选择运动员查看完整数据分�?/div>'; return; }
    ...
    .then(function(d){
        ...
        html += '<div class="viz-stats-grid" style="margin-bottom:14px;">' +
            '<div class="viz-stat-card" style="background:#e8f5e9;"><div class="v-value" style="color:#2e7d32;">'+(d.totalMatches||0)+'</div><div class="v-label">总场�?/div></div>' +
            '<div class="viz-stat-card" style="background:#e8f5e9;"><div class="v-value" style="color:#2e7d32;">'+(d.wins||0)+'</div><div class="v-label">�?/div></div>' +
            '<div class="viz-stat-card" style="background:#ffebee;"><div class="v-value" style="color:#c62828;">'+(d.losses||0)+'</div><div class="v-label">�?/div></div>' +
        ...
        if (d.competitionDetails && d.competitionDetails.length > 0) {
            ...
            if (d.compTypeBreakdown && d.compTypeBreakdown.length > 0) {
                compHtml += '<h4 style="font-size:12px;color:#888;margin:14px 0 6px;">按赛事类型统�?/h4>';
        ...
        if (d.roundBreakdown && d.roundBreakdown.length > 0) {
            ...
            d.roundBreakdown.forEach(function(r){
                if (r.round && r.round.indexOf('�?) >= 0) return;
            ...
            tabSections.push({label:'各轮次胜�?, html:roundHtml});
        }
        ...
        if (hasScoreAnalysis) {
            var scoreHtml = '<div style="display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap;">' +
                '<div class="viz-stat-card" style="flex:1;min-width:100px;background:#e3f2fd;text-align:center;padding:10px;"><div class="v-value" style="font-size:20px;color:#0077b6;">'+(d.maxScoreDiff||0)+'</div><div class="v-label" style="font-size:11px;">单局最大分�?/div></div>' +
...
function loadVizOverview() {
    ...
    .then(function(d){
        ...
        if (d.scoreDistribution && d.scoreDistribution.length > 0) {
            ...
            html += '<h4 style="font-size:14px;color:#333;margin:0 0 8px;">📊 全局比分分布 <span style="font-size:11px;color:#999;font-weight:400;">最大分�? '+(d.maxScoreDiff||0)+'</span></h4>';
        ...
        if (d.mostActivePlayers && d.mostActivePlayers.length > 0) {
            html += '<div style="margin-bottom:20px;">';
            html += '<h4 style="font-size:14px;color:#333;margin:0 0 8px;">👥 最活跃运动�?Top 20</h4>';
            html += '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:12px;"><thead><tr style="background:#f5f7fa;">' +
                '<th style="padding:6px 10px;text-align:center;border-bottom:1px solid #eee;">#</th>' +
                '<th style="padding:6px 10px;text-align:left;border-bottom:1px solid #eee;">运动�?/th>' +
                '<th style="padding:6px 10px;text-align:center;border-bottom:1px solid #eee;">场次</th>' +
                '<th style="padding:6px 10px;text-align:center;border-bottom:1px solid #eee;">�?/th>' +
                '<th style="padding:6px 10px;text-align:center;border-bottom:1px solid #eee;">胜率</th></tr></thead><tbody>';
        ...
        if (d.compTypeDistribution && d.compTypeDistribution.length > 0) {
            ...
            pieDiv.innerHTML = '<h4 style="font-size:14px;color:#333;margin:0 0 8px;">🏟�?各类赛事占比</h4><div id="v-ov-comp-type" style="height:260px;"></div>';
        ...
        if (d.countryChampionships && d.countryChampionships.length > 0) {
            ...
            champDiv.innerHTML = '<h4 style="font-size:14px;color:#333;margin:0 0 8px;">🏆 各国冠军�?/h4><div id="v-ov-country-chart" style="height:280px;"></div>';
        ...
        if (d.upsets && d.upsets.length > 0) {
            ...
            upsetHtml += '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:12px;"><thead><tr style="background:#f5f7fa;">' +
                '<th style="padding:6px 10px;text-align:left;border-bottom:1px solid #eee;">胜�?/th>' +
                '<th style="padding:6px 10px;text-align:center;border-bottom:1px solid #eee;">胜者积�?/th>' +
                '<th style="padding:6px 10px;text-align:center;border-bottom:1px solid #eee;">败者积�?/th>' +
